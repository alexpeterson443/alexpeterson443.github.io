// Private link: visiting /?key=<ACCESS_KEY> once sets a signed cookie for a year.
// The cookie is HMAC(ACCESS_KEY, expiry) so nothing is stored server side.
...
export async function keyMatches(env, supplied) {
  if (!env.ACCESS_KEY || typeof supplied !== "string") return false;
  // Compare HMACs rather than raw strings so lengths never leak.
  const a = await hmac(env.ACCESS_KEY, supplied);
  const b = await hmac(env.ACCESS_KEY, env.ACCESS_KEY);
