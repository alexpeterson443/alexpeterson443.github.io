...
Run it once a day after the close, or on a timer during the session. State is
persisted to JSON so a restart does not forget your open positions.
"""
...
from .core import Action, Bar, Order, OrderType, Side, parse_date
...
class Trader:
    ...
    def fetch(self, as_of: Optional[date] = None) -> Dict[str, List[Bar]]:
        ...
        return data_mod.load_universe(
            self.symbols, start, end, provider=self.provider, csv_dir=self.csv_dir
        )
    ...
    def plan(self, series: Dict[str, List[Bar]]) -> List[dict]:
        ...
        if halt:
            self._say(f"RISK HALT: {halt}. Exiting positions, no new entries.")
            return [
                {"symbol": s, "side": Side.SELL, "qty": q, "price": series[s][-1].close,
                 "reason": "risk halt"}
        ...
        for symbol in self.symbols:
            ...
            last = bars[i]
            in_position = held.get(symbol, 0.0) > 0
            signal = self.strategy.evaluate(symbol, i, in_position)

            if signal.action is Action.HOLD:
                continue
            if signal.is_exit and in_position:
                intents.append({"symbol": symbol, "side": Side.SELL, "qty": held[symbol],
                                "price": last.close, "reason": signal.reason})
            elif signal.is_entry and not in_position:
                atr_series = ind.atr(
                ...
                if qty > 0:
                    intents.append({"symbol": symbol, "side": Side.BUY, "qty": qty,
                                    "price": last.close, "reason": signal.reason})
                else:
    ...
    def step(self, as_of: Optional[date] = None) -> List[dict]:
        ...
        for intent in intents:
            ...
            order = Order(
                ...
                tag=intent["reason"][:24],
            )
            receipt = self.broker.submit(order, intent["price"])
            if receipt:
                self._say(f"SENT {label}")
                executed.append(receipt)
            else:
                self._say(f"REJECTED {label}")
